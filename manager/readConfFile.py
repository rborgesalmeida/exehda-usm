...
class ReadConfFile(object):
    ...
    def __init__(self, confFile):
        ...
        """
        try:
            self.logFile = '/tmp/exehda-usm-manager.log'
    ...
    def validConfFile(self):
        ...
        """
        try:
            if (self.dbRName == None or self.dbRPass == None or self.dbRPort == None 
                or self.dbRServer == None or self.dbRUser == None):
                logging.error("ERRO: Por favor, verifique os parâmetros de configuração " + 
                                  "para o banco de dados e a chave simétrica em " + self.confFile)
                return False
            else:
                return True
        except Exception, e:
        ...
        try:
            ...
            for line in f :
                ...
                if not line.startswith('#') and line != '\n': 
                    ...
                    elif line[0] == 'DBRServer':
                        try:
                            IP(line[1])
                            self.dbRServer = line[1]
                        except Exception:
                            logging.error("ERRO: Parâmetro DBRServer com valor " + line[1] +
                                              " inválido")
                            return False
                    elif line[0] == 'DBRPort':
                        try:
                            if int(line[1]) > 0 and int(line[1]) < 65535:
                                self.dbRPort = int(line[1])
                        except Exception:
                            logging.error("ERRO: Parâmetro DBRPort com valor " + line[1] +
                                              " inválido")
                            return False
                    elif line[0] == 'DBRUser':
                            self.dbRUser = line[1]
