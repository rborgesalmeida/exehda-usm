...
class Esper(Process):
	...
	#self.crypto = crypto
        ...
        self.local = path[0]
        self.itemsFile = str(self.local) + '/esper-5.3.0/exehda-usm-manager/serverItems.csv'
            ...
            if restart == True:
                ...
                try:
                    ...
                    if len(situationsNew) > 0:    
                        setenv = Popen(["sh", str(self.local) + "/esper-5.3.0/exehda-usm-manager/etc/setenv.sh"])
                        setenv.wait()
                        compile_ = Popen(["sh", str(self.local) + "/esper-5.3.0/exehda-usm-manager/etc/compile.sh"])
                        compile_.wait()                
                        cmd = "sh " + str(self.local) + "/esper-5.3.0/exehda-usm-manager/etc/run_namedwin.sh " + str(self.itemsFile) + ' ' + str(self.portEsperSend)
...
#                     identifierEPL = self.between('`', '`', str(situation[2]))
    ...
    def createEsperJavaFile(self, situationsNew):
        ...
        """
        try:
            esperStatementsFile = open(str(self.local) + "/esper-5.3.0/exehda-usm-manager/src/main/java/com/espertech/esperio/socket/EsperSocketDefault.java", "r")
            ...
            rename(str(self.local) + "/esper-5.3.0/exehda-usm-manager/src/main/java/com/espertech/esperio/socket/EsperSocket.java", str(self.local) + "/esper-4.9.0/cpnm-server/src/main/java/com/espertech/esperio/socket/EsperSocketOld.java")
            esperStatementsFile = open(str(self.local) + "/esper-5.3.0/exehda-usm-manager/src/main/java/com/espertech/esperio/socket/EsperSocket.java", "w")
