...
read -e -p "Create EXEHDA-USM relational database and user? [y/n] " -i "y" createdb
if [ "$createdb" = "y" ]; then
...
read -e -p "If you already configured pg_hba.conf, do you want import database? [y/n] " -i "y" importdb
if [ "$importdb" = "y" ]; then
