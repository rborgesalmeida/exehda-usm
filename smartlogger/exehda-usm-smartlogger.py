...
if __name__ == "__main__":
    try:
        ...
        confFile = 'exehda-usm-smartlogger.conf'
	...
	logging.basicConfig(filename=readConf.logFile, format='%(asctime)s debian exehda-usm-smartlogger[%(process)d]: %(message)s',
                            datefmt='%b %d %H:%M:%S', level=logging.INFO)
        logging.info("Inicializando exehda-usm-smartlogger...")
        #=========================================================================================#
        ...
        if dbConnection.verifyCorrelation():
            ...
            procEsper = Esper(readConf.esperPort, managerCommunication)
	...
	logging.info("Fechando exehda-usm-smartlogger...")
        if procEsper != None:
            procEsper.stop()
	    procEsper.join()
        exit(0)
    ...
    # Caso ocorra alguma exceção, está é registrada no log
        ...
        logging.info("exehda-usm-smartlogger finalizado inesperadamente", exc_info=True)
    #=========================================================================================#
