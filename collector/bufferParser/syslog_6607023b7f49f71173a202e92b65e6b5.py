...
priority = Combine(Suppress(menor)+ integer + Suppress(maior))
...
tosPrec = Suppress(Literal("TOS=") + integer + Literal("x") + integer 
...
flag = Suppress(Literal("DF") ^ Literal("MF") ^ Literal("CE"))
...
url = Word(alphas + nums + "/" + "?" + "." + ":" + "_" + "=" + "$" + "&" + "%" + 
                       "-" + "@" + "," + "[" + "]" + "(" + ")" + "+" + "!" + ";" + "*")
protocol = Combine(alpha + bar + integer + point + integer + Literal('"'))
bytes = integer
...
shorewall = Optional(priority.setResultsName("priority"))+(date.setResultsName("date_coll") + hostname.setResultsName("hostname") + 
                         program.setResultsName("program") + Suppress(numProgram) + 
