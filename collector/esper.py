...
from os import rename, linesep, devnull
...
class Esper(multiprocessing.Process):
    '''
    ...
    def __init__(self, hashFileName, portEsperSend, nexthopCommunication, managerCommunication):
        '''
        ...
        self.local = path[0]
        self.itemsFile =  './esper-5.3.0/exehda-usm-collector/' + hashFileName + '.csv'
        self._is_alive = True
	...
	FNULL = open(devnull, 'w')        
        ...
        logging.info('Inicializando processo do Esper: %d', p.pid)
        ...
        while self._is_alive:
            ...
            if restart == True:
                ...
                try:
                    ...
                    if len(situationsNew) > 0 or len(situationsNew) == 0:    
                        setenv = Popen(["sh",  str(self.local) + "/esper-5.3.0/exehda-usm-collector/etc/setenv.sh"])
                        setenv.wait()
                        compile_ = Popen(["sh", str(self.local) + "/esper-5.3.0/exehda-usm-collector/etc/compile.sh"], stdout=FNULL, stderr=FNULL)
                        compile_.wait()                
                        cmd = "sh " + str(self.local) + "/esper-5.3.0/exehda-usm-collector/etc/run_namedwin.sh " + str(self.itemsFile) + ' ' + str(self.portEsperSend)
...
#                                         print "DADOS RECEBIDOS: ", next_line
                                        ...
                                        if len(msgList) != 1:
                                            ...
                                            # Procura se a situação já foi identificada para somar as suas ocorrências
                                            if situationsOccurredDict.has_key(situation.descriptionSubs):
                                                situationMatched = situationsOccurredDict[situation.descriptionSubs]
                                            else:
                                                situationsOccurredDict[situation.descriptionSubs] = deepcopy(situation)
                                                situationMatched = situationsOccurredDict[situation.descriptionSubs]
                                            
                                            situationMatched.occurred = situationMatched.occurred + 1
                                            commandresult = None
                                            if situationMatched.occurrences == 0:
                                               if situationMatched.occurred == 1:                                                   
                                                   commandresult = situationMatched.executeCommand()
                                            elif situationMatched.occurrences == 1:                                                 
                                                commandresult = situationMatched.executeCommand()
                                            
                                            data.append(situationMatched.descriptionSubs)
                                            data.append(events)
                                            data.append(situationMatched.id)
                                            data.append(situationMatched.commentsSubs)
                                            data.append(situationMatched.priority)                                                                            
                                            data.append(situationMatched.occurred)
                                            if commandresult != None:
                                                data.append(commandresult)
                                            else:
                                                data.append('')
                                                        
                                            self.nexthopCommunication.sendSituations("active_situations", formatColumn, data)                                                                     			    
					    logging.info("Situação: %s", str(data[0]))
                                        else:
                                            # só entrará neste else quando não encontrar situações simples
                                            # (de monitoramento de status)                                            
                                            msgList = str(next_line).split('~')
                                            if msgList[0] in self.situationsStatusDict.keys():
						situation = self.situationsStatusDict[msgList[0]]
                                                events = msgList[1]
                                                                                                                                    
                                                self.nexthopCommunication.removeSituations(msgList[0], situation.id, events)                                                                                                                                                   
                except Exception, e:
                    logging.error('Falha ao executar o Esper', exc_info=True)
    ...
    def verifySituations(self, situations, items):
        ...
        if len(situations) != len(situationWithItem):
            logging.warning('Situações foram desativadas pois não há itens de coleta de dados ativos associados à elas')
    ...
    def createDictSituations(self, situations):
        ...
        try:
            ...
            for situation in situations:
                ...
                self.situationsDict['@Priority (' + str(priority) + ') @Drop ' + situation[2]] = Situations(situation[0], situation[1], situation[2], 
                                                                   situation[3], situation[4], situation[5],
                                                                   situation[6], situation[7], situation[8], 
                                                                   priority, situation[14])
...
#                     identifierEPL = self.between('`', '`', str(situation[2]))
            ...
            logging.error('Falha ao criar dicionário com situações', exc_info=True)     
    ...
    def createEsperJavaFile(self, situationsNew):
        ...
        """
        try:
            esperStatementsFile = open(str(self.local) + "/esper-5.3.0/exehda-usm-collector/src/main/java/com/espertech/esperio/socket/EsperSocketDefault.java", "r")
            ...
            for line in lines:
                if "// Statements" not in line:
                    ...
                    for situation in situationsNew:
                        priority = situation[9] * situation[13]
                        newLines.append("\tEPStatement stmt" + str(i) + ' = provider.getEPAdministrator().createEPL("@Priority (' + str(priority) + ') @Drop ' +  str(situation[2]) + '");\n')
                        newLines.append("\tstmt" + str(i) + '.addListener(new CEPListener("L' + str(i) + '" ));\n')
                        i = i + 1
            esperStatementsFile.close()
            rename(str(self.local) + "/esper-5.3.0/exehda-usm-collector/src/main/java/com/espertech/esperio/socket/EsperSocket.java", str(self.local) + "/esper-5.3.0/cpnm-agent/src/main/java/com/espertech/esperio/socket/EsperSocketOld.java")
            esperStatementsFile = open(str(self.local) + "/esper-5.3.0/exehda-usm-collector/src/main/java/com/espertech/esperio/socket/EsperSocket.java", "w")
            esperStatementsFile.writelines(newLines)
            esperStatementsFile.close()
        except Exception, e:
            logging.error('Falha ao criar arquivo java para o esper', exc_info=True)
    ...
    def getItemsToEsper(self):
        ...
        '''
        try:
            # Realiza a descoberta de itens para posterior coleta de itens (caso existam
            # (novos itens a serem monitorados, as situações a serem detectadas, e seus
            # dados (formato das colunas) já estarão prontos)
            ...
            logging.error('Falha ao adquirir itens', exc_info=True)
    ...
    def itemsProperty(self, monitoredItems, type_):
        ...
        try:        
            ...
            logging.error('Falha ao selecionar as propriedades dos itens a serem repassadas ao esper', exc_info=True)
